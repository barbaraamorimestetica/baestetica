...
**O procedimento de regeração está em [`../[a documentação do Tailwind]`](../[a documentação do Tailwind])**, que é
...
```bash
python [a conferência de classes]
```

## Os outros três — escritos à mão

- **`tokens.css`** — paleta nomeada, tipografia, escala base. **Comum ao
  repositório privado**: depois de mexer, corra `python [o sincronizador]`.
  Ver [`../[a documentação comum]`](../[a documentação comum]).
...
vem no fim e ganha do `<style>` da página. A tabela está em `../[a documentação comum]`.
